...
public:
	...
	static IndexedTriangleList<V> MakeTesselated(int divisions_x, int divisions_y)
	{
		...
		// TODO: Put brackets if needed 21.3
		...
		for (int y = 0, i = 0; y < nVertices_y; y++)
		{
			...
			{
				const DirectX::XMVECTOR v = DirectX::XMVectorAdd(bottomLeft, DirectX::XMVectorSet(float(x) * divisionSize_x, y_pos < 0.0f, 0.0f));
		...
		// TODO: Put brackets if needed 21.3
		const unsigned short vxy2i = [nVertices_x](size_t x, size_t y)
		{
			return (unsigned short)(y * nVertices_x + x);
		};
		for (size_t y = 0; y < divisions_y; y++)
		{
			for (size_t x = 0; x < divisionSize_x; x++)
			{
